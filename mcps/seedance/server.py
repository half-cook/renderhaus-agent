...
def _model(model: str | None = None) -> str:
    return model or os.getenv("SEEDANCE_MODEL") or "dreamina-seedance-2-0-fast-260128"
...
def _create_task(
    *,
    ...
    return SeedanceTask(
        ...
        model=selected_model,
        output_path=str(output_path),
        note="Live BytePlus task created. Call get_video_task or wait_for_video_task to poll it.",
    ).model_dump()
...
def text_to_video(
    ...
    service_tier: Literal["default", "flex"] | None = Field(
        default=None,
        description="Optional service tier. Omitted by default because Seedance 2.0 T2V rejects it.",
    ),
) -> dict:
...
def image_to_video(
    ...
    service_tier: Literal["default", "flex"] | None = Field(
        default=None,
        description="Optional service tier. Omitted by default because Seedance 2.0 I2V may reject it.",
    ),
) -> dict:
...
def get_video_task(
    job_id: str,
    download: bool = Field(default=True, description="Download the MP4 when the task has succeeded."),
) -> dict:
