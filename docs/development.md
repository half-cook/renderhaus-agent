...
3. `make web` → open http://127.0.0.1:8000
4. Change code → restart web if needed → retry.
...
3. **Web** — `/api/productions` + Production tab in the UI (plan → approve → run).
...
In the UI: choose **Production**, enter a brief, **Plan production**, then **Approve & run**.
...
| Both | **All** |
| Only UI (`web/**`) | Nothing cloud — local is enough |
